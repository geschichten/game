...
"use strict"; var Imported = Imported || {}; var Yanfly = Yanfly || {}; // In case there's no Yanfly plugins in use
...
} if (parseFloat(OcRam.version) < 1.04) alert("OcRam core v1.04 is required!");
...
"use strict"; if (!Imported || !Imported.OcRam_Core) alert('OcRam_Core.js ' +
    'is required!'); OcRam.addPlugin("Layers", "2.01");

/*:
 * @plugindesc v2.01 Display layers like fog or sunray. Layers can be inherited
 ...
 * 
 * Plugin command: 
 * layer indx imgName scrollX scrollY opacity fixToMap fade loop anim
 *
 ...
 *      layer 1 fog 0.3 -0.2 1 false 3 loop:1,1 anim:2,500
 *
 * fixed to screen, not scrolling nor looping foreground with 'sunrays':
 *      layer 2 sunrays 0 0 false 2 loop:0,0
 *
 ...
 *          (use any region restriction plugin to do collision map)
 *      2 = Above characters
 *
 ...
 * ALL of my plugins for 1 project = 39.99 EUR (standard licenses)
 *
...
OcRam_Layer.prototype.applyParams = function (indx, pic_name, sx, sy, o, f2m, fade, lx, ly, anim, rate) {
...
OcRam_Layer.prototype.applyNewLayer = function () {
    ...
    this._opacity = 0; this._sprite.opacity = 0;
    ...
    if ($gameMap && this._fixedToMap) {
        this._x = $gameMap._displayX * 2;
        this._y = $gameMap._displayY * 2;
    } else {
        this._x = 0; this._y = 0;
...
(function () {
    ...
    this.extend(Game_Interpreter, "pluginCommand", function (command, args) {
        switch (command) {
            ...
            case "layer": case "oc_layer": // indx, img_name, sx, sy, o, f2m, fade, loop, anim
                setLayer(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7], args[8]); break;
    ...
    Game_Map.prototype.setLayer_OC = function (indx, img_name, sx, sy, o, f2m, fade, loop_xy, anim) {
        setLayer(indx, img_name, sx, sy, o, f2m, fade, loop_xy, anim);
    }; Game_Screen.prototype.setLayer_OC = function (indx, img_name, sx, sy, o, f2m, fade, loop_xy, anim) {
        setLayer(indx, img_name, sx, sy, o, f2m, fade, loop_xy, anim);
    ...
    Game_Map.prototype.scrollY_OC = function (distance, difference) {
        if (this.isLoopVertical()) {
            for (var i = 0; i < 3; i++) {
                if (_allLayers[i]._loopY) { _allLayers[i]._y -= distance; }
            }
    ...
    Game_Map.prototype.scrollX_OC = function (distance, difference) {
        if (this.isLoopHorizontal()) {
            for (var i = 0; i < 3; i++) {
                if (_allLayers[i]._loopX) _allLayers[i]._x += distance;
            }
    ...
    this.extend(Game_System, "onBeforeSave", function () {
        ...
        oc_sprites.forEach(function (sprite) { // id, pic_name, x, y, z, o, a, anim
            ...
            obj._scrollX = layer._scrollX; obj._scrollY = layer._scrollY;
            obj._opacity = layer._opacity; obj._fixedToMap = layer._fixedToMap;
            obj._loopX = layer._loopX; obj._loopY = layer._loopY; obj._fadeTime = layer._fadeTime;
            obj._animFrames = layer._animFrames; obj._animRate = layer._animRate;
    ...
    this.extend(Game_System, "onAfterLoad", function () {
        ...
        if (this._OC_Layers) {
            this._OC_Layers.forEach(function (l) { // indx, img_name, sx, sy, o, f2m, fade, loop, anim
                setLayer(l._z, l._imgName, l._scrollX, l._scrollY, l._opacity, l._fixedToMap, 256, "loop:" + l._loopX + "," + l._loopY, "anim:" + l._animFrames + "," + l._animRate);
            });
        } if (this._OC_Sprites) {
            this._OC_Sprites.forEach(function (s) { // pic_name, x, y, z, o, a
                addSprite(s._orgId, s._orgName, s._orgX, s._orgY, s._orgZ, s._orgOpacity, s._orgAlign, s._orgAnim);
            });
        }
    ...
    this.extend(Scene_Map, "start", function () {
        ...
        if (_gameSysLoading) $gameSystem.loadLayerData(); _gameSysLoading = false;
    });
    ...
    function setLayer(indx, img_name, sx, sy, o, f2m, fade, loop_xy, anim) {

        _this.debug("setLayer", "(" + indx + ", " + img_name + ", sx:" + sx + ", sy:" + sy + ", o:" + o + ", f2m:" + f2m + ", fade:" + fade + ", loop=" + loop_xy + ", anim=" + anim + ")");
        ...
        _this.debug("REGULATED VALUES: setLayer", "(" + indx + ", " + img_name + ", sx:" + sx + ", sy:" + sy + ", o:" + o + ", f2m:" + f2m + ", fade:" + fade + ", lx:" + lx + ", ly:" + ly + ", ac:" + ac + ", ar:" + ar + ")");

        this_layer.applyParams(indx, img_name, sx, sy, o, f2m, fade, lx, ly, ac, ar); // Apply values
            ...
            if (layer._sprite.bitmap) { // Layer is valid!
                ...
                args.forEach(function (p) {
                    ...
                    switch (par) {
                        ...
                        case "opacity": layer._opacity = parseFloat(val); layer._targetOpacity = parseFloat(val); break;
    ...
    function addSprite(id, pic_name, x, y, z, o, a, anim) {
        ...
        z = parseInt(z) * 2; x = Number(x); y = Number(y);
